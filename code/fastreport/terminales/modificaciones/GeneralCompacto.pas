...
var
  ...
  lEscribiMosq: Boolean; dir, desc, code: string; partsIdx: Integer;
...
try
  ...
  // Si NO hay sección o está vacía, mensaje directo
  if (lSL.Count = 0) then
  begin
    lTexto.Add('');
    lTexto.Add('No lleva mosquitera');
  end
  else
  begin
    // Recorremos todas las entradas y filtramos por el rectángulo actual: R<lRectangulo>.*
    for I := 0 to lSL.Count - 1 do
    begin
      if Copy(lSL.Strings[I], 1, Length('R'+IntToStr(lRectangulo)+'.')) <>
         'R'+IntToStr(lRectangulo)+'.' then
        Continue;

      // parts[3] → grupo/orientación: 1=Superior, 3=Inferior, 5=Derecha, 7=Izquierda
      lUso := DameParametro(lIni.ReadString(lCad, lSL.Strings[I], ''), 4); // cuarto campo
      try
        lSituacion := StrToInt(lUso);
      except
        lSituacion := 0;
      end;
      ...
      case lSituacion of
        ...
        Continue; // ignorar otros grupos
      end;

      // Cabecera "MOSQUITERA" solo la primera vez que haya algo válido
      if lTexto.IndexOf('MOSQUITERA') < 0 then
      begin
        lTexto.Add('');
        lTexto.Add('MOSQUITERA');
      end;

      // Código (referencia) y descripción
      lImg  := DameParametro(lIni.ReadString(lCad, lSL.Strings[I], ''), 1); // código/ref
      lCad2 := DameParametro(lIni.ReadString(lCad, lSL.Strings[I], ''), 5)  // descripción
               + ' (' + lImg + ') - ' + lDato;

      if lTexto.IndexOf(lCad2) < 0 then
      begin
        lTexto.Add(lCad2);
        InsertaImagen(lImg); // opcional, igual que haces en otros bloques
      end;
    end;

    // Si la sección existía pero no había R<rectángulo>.* válidos: mensaje
    if lTexto.IndexOf('MOSQUITERA') < 0 then
    begin
      lTexto.Add('');
      lTexto.Add('No lleva mosquitera');
