var
  ...
  // Variables para control de columnas y filas
...
begin
  ...
  // Control de columnas y filas (opcional - limitar a X filas)
  MaxColumns := 9;  // 9 columnas configuradas en Page1
  MaxRows := 5;     // Máximo 5 filas (ajusta según necesites)
  
  CurrentColumn := (EstadoLineasLoteT.RecNo - 1) mod MaxColumns;
  CurrentRow := (EstadoLineasLoteT.RecNo - 1) div MaxColumns;
  
  // Ocultar si supera el número máximo de filas
...
var
  lPicture: TfrxPictureView;
  lStream: TMemoryStream;
  lField: TField;
begin
  lPicture := TfrxPictureView(Sender);
  lPicture.Picture := nil;  // Limpiar primero
  
  try
    // Acceder directamente al campo del dataset actual
    lField := EstadoLineasLoteT.FieldByName('Grafico');
    
    if (lField <> nil) and (not lField.IsNull) then
    begin
      lStream := TMemoryStream.Create;
      try
        // Guardar el BLOB en el stream
        TBlobField(lField).SaveToStream(lStream);
        
        if lStream.Size > 0 then
        begin
          lStream.Position := 0;
          lPicture.Picture.LoadFromStream(lStream);
        end;
      finally
        lStream.Free;
      end;
    ...
    // Si hay cualquier error, dejar la imagen vacía
    lPicture.Picture := nil;
  end;
end;
