...
function DameSeccionMonoblock(pIni : TMemIniFile) : String;
//Determina si usar MONOBLOK o COMPACTOMB según lo que exista en el INI
var
  lSL : TStringList;
begin
  Result := 'MONOBLOK'; // Por defecto intentamos MONOBLOK

  lSL := TStringList.Create;
  try
    pIni.ReadSection('MONOBLOK', lSL);
    if lSL.Count > 0 then
    begin
      Result := 'MONOBLOK';
      Exit;
    ...
    // Si MONOBLOK no existe o está vacío, probamos COMPACTOMB
    lSL.Clear;
    pIni.ReadSection('COMPACTOMB', lSL);
    if lSL.Count > 0 then
    begin
      Result := 'COMPACTOMB';
      Exit;
    ...
    // Si ninguna existe, devolvemos MONOBLOK por defecto
    Result := 'MONOBLOK';
  finally
    lSL.Free;
  end;
...
var
  ...
  lEscribiMosq: Boolean;
  dir: string;
  partsIdx: Integer;



  v: String;                 // valor completo de la línea "Rxx.yy=...." del INI
  code, desc: String;        // referencia (parts[1]) y descripción (parts[5])
  tuboCode, tuboDesc: String; // referencia y desc del tubo (ITE42023)
  torCode, torDesc: String;   // referencia y desc del tornillo (TOR4220)
  n: Integer;                // grupo/orientación (parts[4] -> 1/2/3/4)
  // Si en tu procedimiento NO tienes ya el índice de bucle:
 // i: Integer;                // índice de los bucles del bloque de recercado
  ...
  lHoja:='3';
   ...
   // MemoInformacionTexto.Text := 'Buscando Informacion necesaria a mostrar en Compacto';
     ...
     Begin
      ...
      If I2=4 then lCad:=DameSeccionMonoblock(lIni);
    ...
    If (TAREAENCURSO='COMPACTO') and (SECCION<>'MARCOS') and (SECCION<>DameSeccionMonoblock(lIni)) then
      Begin
       LMENSAJES:='EN EL PUESTO DE MARCOS SOLO SE PUEDEN PROCESAR MARCOS O ' + DameSeccionMonoblock(lIni) + #13#10 + 'La etiqueta escaneada de corresponde con: '+ #13#10 + SECCION;
...
begin
  ...
  lSL.Free;
   ...
   If TAREAENCURSO='COMPACTO' then
    Begin
      //EL CAJON
      lCad:=DameSeccionMonoblock(lIni);
      lSL := TStringList.Create;
      lIni.ReadSection(lCad,lSL);
      If (lSL.Count<>0) then
        Begin
...
try
  ...
  begin
    lTexto.Add('');
    lTexto.Add('No lleva recercado');
  end
  else
  begin
    if lTexto.IndexOf('RECERCADO PARA MOSQUITERA') < 0 then
    begin
      lTexto.Add('');
      lTexto.Add('RECERCADO PARA MOSQUITERA');
    end;

    // ---- D E R E C H A  (grupo = 4) ----
    tuboCode := ''; torCode := '';
    for i := 0 to lSL.Count-1 do begin
      v := lIni.ReadString(lCad, lSL.Strings[i], '');
      try n := StrToInt(DameParametro(v,4)); except n := 0; end;
      if n<>4 then Continue;
      code := DameParametro(v,1);
      desc := DameParametro(v,5);
      if UpperCase(code)='ITE42023' then begin tuboCode:=code; tuboDesc:=desc; end;
      if UpperCase(code)='TOR4220'  then begin torCode :=code; torDesc :=desc; end;
    end;
    if (tuboCode<>'') and (torCode<>'') then
    begin
      lTexto.Add(tuboDesc+' ('+tuboCode+') - '+torDesc+' ('+torCode+') - Derecha');
      InsertaImagen(tuboCode); InsertaImagen(torCode);
    end;

    // ---- I Z Q U I E R D A  (grupo = 2) ----
    tuboCode := ''; torCode := '';
    for i := 0 to lSL.Count-1 do begin
      v := lIni.ReadString(lCad, lSL.Strings[i], '');
      try n := StrToInt(DameParametro(v,4)); except n := 0; end;
    ...
    if (tuboCode<>'') and (torCode<>'') then
    begin
      lTexto.Add(tuboDesc+' ('+tuboCode+') - '+torDesc+' ('+torCode+') - Izquierda');
      InsertaImagen(tuboCode); InsertaImagen(torCode);
    end;

    // ---- S U P E R I O R  (grupo = 1) ----
    tuboCode := ''; torCode := '';
    for i := 0 to lSL.Count-1 do begin
      v := lIni.ReadString(lCad, lSL.Strings[i], '');
      try n := StrToInt(DameParametro(v,4)); except n := 0; end;
    ...
    if (tuboCode<>'') and (torCode<>'') then
    begin
      lTexto.Add(tuboDesc+' ('+tuboCode+') - '+torDesc+' ('+torCode+') - Superior');
      InsertaImagen(tuboCode); InsertaImagen(torCode);
    end;

    // ---- I N F E R I O R  (grupo = 3) ----
    tuboCode := ''; torCode := '';
    for i := 0 to lSL.Count-1 do begin
      v := lIni.ReadString(lCad, lSL.Strings[i], '');
      try n := StrToInt(DameParametro(v,4)); except n := 0; end;
    ...
    if (tuboCode<>'') and (torCode<>'') then
    begin
      lTexto.Add(tuboDesc+' ('+tuboCode+') - '+torDesc+' ('+torCode+') - Inferior');
      InsertaImagen(tuboCode); InsertaImagen(torCode);
    end;

    // si no salió ninguna línea válida
    if (lTexto.IndexOf('RECERCADO PARA MOSQUITERA') >= 0) and
       (lTexto[lTexto.Count-1] = 'RECERCADO PARA MOSQUITERA') then
    begin
      lTexto.Delete(lTexto.Count-1); // quitar cabecera vacía
      lTexto.Add('');
      lTexto.Add('No lleva recercado');
    end;
  end;
...
//{ ==== FIN MOSQUITERA ==== }
        ...
        // Mostrar JAMBA (solapes / tapajuntas) en la tarea COMPACTO
    Begin
      //EL TAPAJUNTAS
      lCad:='JAMBA';
      lSL := TStringList.Create;
    ...
    // Mostrar TAPAJUNTAS solo si existe la sección JAMBA en el INI
    // ...existing code...
