...
begin
  ...
  // Compatibilidad: si la tarea viene como ESPECIALES, tratarla como ARMADO
  // para mostrar la información de HUECOS/ARMADO asociada a los rectángulos
  // (rect. 9,10,11,12, etc.). Esto evita que solo se muestre el encabezado
  // "TAREA.....ESPECIALES" cuando no hay un bloque específico para ESPECIALES.
  if TAREAENCURSO = 'ESPECIALES' then
    TAREAENCURSO := 'ARMADO';
...
begin
   ...
   If TAREAENCURSO='ARMADO' then
    Begin
      ...
      lSl.Free;
                ...
                lSL2.Free;
                         lTexto.Add('');
                         // Eliminado: no se solicita información de 'CARRO CASILLA JUNQUILLOS'
  ...
  Begin
    ...
    // Comprobar si existe la seccion MONOBLOCK y mostrarla
    lCad := 'MONOBLOCK';
    lSL := TStringList.Create;
    lIni.ReadSection(lCad,lSL);
    If (lSL.Count>0) then
    Begin
      lTexto.Add('');
      lTexto.Add('MONOBLOCK');
      For I := 0 to lSL.Count - 1 do
      begin
        // Tomo el nombre/descripcion en parametro 5 y el identificador en parametro 1
        lCad2 := DameParametro(lIni.ReadString(lCad,lSL.Strings[I],''),5) + ' - (' + DameParametro(lIni.ReadString(lCad,lSL.Strings[I],''),1) + ')';
        If lTexto.IndexOf(lCad2) < 0 then
        Begin
          lTexto.Add(lCad2);
