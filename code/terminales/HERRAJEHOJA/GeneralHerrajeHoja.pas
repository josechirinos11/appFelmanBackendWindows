...
var
  ...
  lIni : TMemIniFile;
  lTexto : TStringList;
  lCad,lCad2, lCad3, lDesc, lDato,lUso,lHoja,lCodbarrascompleto : String;
  lID : String;
  lSL2 : TStringList;
  ...
  lSituacion : Integer;
  varHayHerrajeIzq, varHayHerrajeDer: Boolean;
    ...
    lSL.Free;
      ...
      If (lSL.Count<>0) then
        Begin
         ...
         // Agregar Palanc.Rebajo si está en HERRAJE
         lSL2 := TStringList.Create;
         lIni.ReadSection('HERRAJE', lSL2);
         For I2 := 0 to lSL2.Count - 1 do
         begin
           lCad2 := DameParametro(lIni.ReadString('HERRAJE', lSL2.Strings[I2], ''), 1);
           lDesc := DameParametro(lIni.ReadString('HERRAJE', lSL2.Strings[I2], ''), 5);
           if (lCad2 = 'GUG221830001') or (Pos('Palanc.Rebajo', lDesc) > 0) then
           begin
             lCad3 := lDesc + '    (' + lCad2 + ')';
             if lTexto.IndexOf(lCad3) < 0 then
             begin
               lTexto.Add(lCad3);
               InsertaImagen(lCad2);
             end;
...
BEGIN
  ...
  // Si la sección es HOJA_COR, mostrar herrajes de corredera
  If (SECCION='HOJA_COR') then
  Begin
    // Herraje izquierda
    lSL := TStringList.Create;
    lIni.ReadSection('HER_HIZQ', lSL);
    varHayHerrajeIzq := lSL.Count > 0;
    if varHayHerrajeIzq then
    begin
      lTexto.Add('');
      lTexto.Add('HERRAJE HOJA CORREDERA IZQUIERDA');
      For I := 0 to lSL.Count - 1 do
      Begin
        lCad2 := DameParametro(lIni.ReadString('HER_HIZQ', lSL.Strings[I], ''), 5) + '    (' +
                 DameParametro(lIni.ReadString('HER_HIZQ', lSL.Strings[I], ''), 1) + ')';
        lTexto.Add(lCad2);
        lCad2 := DameParametro(lIni.ReadString('HER_HIZQ', lSL.Strings[I], ''), 1);
        InsertaImagen(lCad2);
      End;
    end;
    lSL.Free;
    ...
    // Herraje derecha
    lSL := TStringList.Create;
    lIni.ReadSection('HER_HDER', lSL);
    varHayHerrajeDer := lSL.Count > 0;
    if varHayHerrajeDer then
    begin
      lTexto.Add('');
      lTexto.Add('HERRAJE HOJA CORREDERA DERECHA');
      For I := 0 to lSL.Count - 1 do
      Begin
        lCad2 := DameParametro(lIni.ReadString('HER_HDER', lSL.Strings[I], ''), 5) + '    (' +
                 DameParametro(lIni.ReadString('HER_HDER', lSL.Strings[I], ''), 1) + ')';
        lTexto.Add(lCad2);
        lCad2 := DameParametro(lIni.ReadString('HER_HDER', lSL.Strings[I], ''), 1);
        InsertaImagen(lCad2);
      End;
    end;
    lSL.Free;

    // Si no hay herrajes en ninguna de las dos secciones, mostrar HERRAJE
    if (not varHayHerrajeIzq) and (not varHayHerrajeDer) then
    begin
      lSL := TStringList.Create;
      lIni.ReadSection('HERRAJE', lSL);
      if lSL.Count > 0 then
      begin
        lTexto.Add('');
        lTexto.Add('HERRAJE DE HOJA CORREDERA');
        For I := 0 to lSL.Count - 1 do
        begin
          lCad2 := DameParametro(lIni.ReadString('HERRAJE', lSL.Strings[I], ''), 5) + '    (' +
                   DameParametro(lIni.ReadString('HERRAJE', lSL.Strings[I], ''), 1) + ')';
          lTexto.Add(lCad2);
          lCad2 := DameParametro(lIni.ReadString('HERRAJE', lSL.Strings[I], ''), 1);
          InsertaImagen(lCad2);
        end;
      end;
      lSL.Free;
  ...
  Begin
    ...
    begin
      ...
      Begin
      // Excluir Bisagra Oculta Hoja 16mm Regul si es hoja activa
        If (SECCION='HOJA_ACT') and (lDato='GU6342290001') then continue;
        // Excluir Bisagra Al Canto Uni-Jet si no es hoja activa
        If (SECCION<>'HOJA_ACT') and (lDato='GU6361800101') then continue;
        lCad2:=   DameParametro(lIni.ReadString(lCad,lSL.Strings[I],''),5)+' ('+DameParametro(lIni.ReadString(lCad,lSL.Strings[I],''),1)+'-'+IntToStr(lSituacion)+')';
        If (lTexto.IndexOf(lCad2) < 0) then
        Begin
          lTexto.Add(lCad2);
