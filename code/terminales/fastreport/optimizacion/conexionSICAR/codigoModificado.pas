...
begin
  ...
  begin
    ...
    var
      ...
      NumImg : Integer = 0;
      RutaImg : String = '\\SERVER-M1\Compartido\Sicar\datos\z_felman2023\imagenes\'; //Debe terminar en "\"
      sqlSicar2: TfrxDBDataset; // Declaración global para consultas SQL
...
// Consulta SQL general e impresión independiente de la tarea
procedure ConsultaComponenteSicar2;
var
  sqlSicar2: TfrxDataset;
  serie: String;
  numero, linea, id: Integer;
begin
  // Mostrar mensaje de espera en Memo40
  Memo40.Text := 'Espera...';

  // Asignar valores según contexto
  serie := <LotesLineasDS_FRT2."OrigenSerie">;
  numero := <LotesLineasDS_FRT2."OrigenNumero">;
  linea := <LotesLineasDS_FRT2."OrigenLinea">;
  id := <Id>;

  sqlSicar2.SQL.Text := 'select c.* from fpresupuestoslineascomponentes c ' +
    'where c.CodigoSerie="' + serie + '"' +
    ' and c.CodigoNumero=' + IntToStr(numero) +
    ' and c.Linea=' + IntToStr(linea) +
    ' and c.Id=' + IntToStr(id) + ' limit 1';

  if (sqlSicar2.Active = false) then
    sqlSicar2.Open;

  // Imprimir resultado en modo general y mostrar columna Informacion
  if (sqlSicar2.RecordCount > 0) then
  begin
    MemoTotal.Text := MemoTotal.Text + chr(10) + 'Consulta General: ' + sqlSicar2.SQL.Text;
    // Buscar y mostrar la columna Informacion si existe
    if (<sqlSicar2."Informacion"> <> '') then
      Memo40.Text := <sqlSicar2."Informacion">
    else
      Memo40.Text := 'Sin información disponible.';
  end
  else
  begin
    MemoTotal.Text := MemoTotal.Text + chr(10) + 'Sin resultados: ' + sqlSicar2.SQL.Text;
    Memo40.Text := 'Sin resultados.';
  end;
end;
